...
public:
    ...
    void InsertRecord(const Type& newItem);
};
...
//MACHINE PROBLEM FUNCTIONS DEFINITION

template <class Type>
void unorderedLinkedList<Type>::InsertRecord(const Type& newItem)
{
    nodeType<Type>* newNode; //pointer to create the new node
...
template <class Type>
void linkedListType<Type>::ShowRecords() const
{
    nodeType<Type>* current; //pointer to traverse the list
    nodeType<Type>* trailCurrent;
    int nameCount;
    const int DAY_COUNT = 14;
    const int SYMPTOM_COUNT = 3;
    int control = 1;
